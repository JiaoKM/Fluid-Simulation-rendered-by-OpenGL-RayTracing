...
class ParticleSystem:
    def __init__(self, config: SimConfig, GGUI=False):
        ...
        if self.cfg.get_cfg("supportRadius"):
            self.support_radius = self.cfg.get_cfg("supportRadius")
        ...
        print(f"Number of fluid bodies and rigid blocks: {num_fluid_object}")
        ...
        self.particle_materials = ti.field(dtype=float, shape=self.particle_max_num)
        ...
        self.object_num[None] = num_fluid_object + num_rigid_object + (1 if self.add_domain_box else 0)
    ...
    def insert_object(self):
        ...
        for fluid_body in self.fluid_bodies:
            ...
            self.add_particles(obj_id,
                               ...
                               1 * np.ones(num_particles_obj, dtype=np.int32),
    ...
    def compute_rigid_body_mass(self, object_id: int) -> ti.f32:
        ...
        for p_i in range(self.particle_num[None]):
            if self.particle_object_ids[p_i] == object_id and self.particle_is_dynamic[p_i]:
                sum_m += self.particle_densities[p_i] * self.m_V0
    ...
    def compute_rigid_body_center_of_mass(self, object_id: int) -> ti.types.vector(3, float):
        ...
        for p_i in range(self.particle_num[None]):
            if self.particle_object_ids[p_i] == object_id and self.particle_is_dynamic[p_i]:
                sum_xm += self.particle_positions[p_i] * self.particle_densities[p_i] * self.m_V0
                sum_m += self.particle_densities[p_i] * self.m_V0
    ...
    def add_particle(self, p, obj_id, x, v, density, pressure, material, is_dynamic, color):
        ...
        self.particle_rest_volumes[p] = self.m_V0
    ...
    def for_all_neighbors(self, p_i, task: ti.template(), ret: ti.template()):
        ...
        for offset in ti.grouped(ti.ndrange(*((-1, 2),) * self.dim)):
            ...
            for p_j in range(start_idx, end_idx):
                if p_i != p_j and (self.particle_positions[p_i] - self.particle_positions[p_j]).norm() < self.support_radius:
                    task(p_i, p_j, ret)
    ...
    def load_fluid_body(self, fluid_body, pitch=None):
        ...
        rotation_matrix = tm.transformations.rotation_matrix(angle, direction, mesh.vertices.mean(axis=0))
        ...
        positions = np.array(np.meshgrid(*num_dim, sparse=False, indexing='ij'), 
                             dtype=np.float32)
        positions = positions.reshape(-1,
                                      reduce(lambda x, y: x * y, 
                                             list(positions.shape[1:]))).transpose()
        
        print(f"processing {len(positions)} points to decide whether they are inside the mesh. This might take a while.")
        inside = [False for _ in range(len(positions))]

        pbar = tqdm(total=len(positions))
        for i in range(len(positions)):
            if mesh.contains([positions[i]])[0]:
                inside[i] = True
            pbar.update(1)
    ...
    def compute_box_particle_num(self, lower_corner, cube_size, thickness, space=None):
        ...
        num_dim = []
        for i in range(self.dim):
            num_dim.append(np.arange(lower_corner[i], lower_corner[i] + cube_size[i], 
                                     space))
        ...
        for i in range(self.dim):
            mask = mask | ((positions[:, i] <= lower_corner[i] + thickness) | (positions[:, i] >= lower_corner[i] + cube_size[i] - thickness))
