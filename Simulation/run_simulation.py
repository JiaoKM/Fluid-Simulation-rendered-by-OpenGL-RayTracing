...
from WCSPH import WCSPH_Solver
...
if __name__ == "__main__":
    ...
    parser.add_argument('--scene_file',
                        default='./scene/LTH_bath_wcsph.json',
                        help='scene file')
    ...
    if simulation_method == "wcsph":
        particle_system = WCSPH_ParticleSystem(config, GGUI=True)
        solver = WCSPH_Solver(particle_system)
    else:
    ...
    camera = ti.ui.Camera()
    camera.position(4.0, 2.5, 7.5)
    camera.up(0.0, 1.0, 0.0)
    camera.lookat(4.0, 2.5, 0.0)
    camera.fov(90)
    ...
    while window.running:
        solver.step()
        particle_system.copy_to_vis_buffer(invisible_objects=invisible_objects, dim=dim)
        if particle_system.dim == 3:
            scene.set_camera(camera)
            scene.point_light((2.0, 2.0, 2.0), color=(1.0, 1.0, 1.0))
            scene.particles(particle_system.x_vis_buffer, radius=particle_system.particle_radius, per_vertex_color=particle_system.color_vis_buffer)
        ...
        if cnt % output_interval == 0:
            if output_ply:
                os.makedirs(f"{scene_name}_output/{cnt:06}", exist_ok=True)
                for f_body_id in particle_system.object_id_fluid_body:
                    obj_data = particle_system.dump(obj_id=f_body_id)
                    np_pos = obj_data["position"]
                    writer = ti.tools.PLYWriter(num_vertices=particle_system.object_collection[f_body_id]["particleNum"])
                    writer.add_vertex_pos(np_pos[:, 0], np_pos[:, 1], np_pos[:, 2])
                    writer.export_ascii(f"{scene_name}_output/{cnt:06}/particle_object_{f_body_id}.ply")
            if output_obj:
                os.makedirs(f"{scene_name}_output/{cnt:06}", exist_ok=True)
                for r_body_id in particle_system.object_id_rigid_body:
                    with open(f"{scene_name}_output/{cnt:06}/mesh_object_{r_body_id}.obj", "w") as f:
                        e = particle_system.object_collection[r_body_id]["mesh"].export(file_type='obj')
                        f.write(e)
