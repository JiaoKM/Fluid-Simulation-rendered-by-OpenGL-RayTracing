...
from utils.urdf import create_urdf

class PyBulletSolver():
    def __init__(self, particle_system: ParticleSystem, gravity: Tuple[float, float, float] = (0, -9.8, 0), dt: float = 1e-3):
        self.container = particle_system
        self.total_time = 0.0
        self.present_rigid_object = []
        assert particle_system.dim == 3, "PyBulletSolver only supports 3D simulation currently"
